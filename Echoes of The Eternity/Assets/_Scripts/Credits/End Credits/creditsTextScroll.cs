...
public class creditsTextScroll : MonoBehaviour
{
    ...
    [Header("Audio")]
    public AudioSource[] creditsMusicSources; // Music to play during credits
    ...
    void Start()
    {
        ...
        creditsMusicSources[0].Play(); // Start playing the credits music
    ...
    private void Update()
    {
        if (checkforsongend)
        {
            ...
            if (!creditsMusicSources[currentMusicIndex].isPlaying)
            {
                creditsMusicSources[currentMusicIndex].Stop(); // just in case.
                currentMusicIndex++;
                if (currentMusicIndex >= creditsMusicSources.Length)
                {
                    currentMusicIndex = 0; // Loop back to the first track
                }
                creditsMusicSources[currentMusicIndex].Play();
            }
        }
    }
