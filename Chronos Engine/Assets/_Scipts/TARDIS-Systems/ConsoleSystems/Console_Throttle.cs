...
public class Console_Throttle : TARDISSubsystemController
{
    ...
    {
        if (!_isCircuitActive || !isFunctional) // Only adjust if control is active and functional
        {
            Debug.Log("Throttle: Cannot increase, control is disengaged or not functional.");
            return;
        }
